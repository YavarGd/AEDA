...
public sealed class AvaloniaDashboardBehaviorTests
{
    [Fact]
    ...
    public async Task Dashboard_TaskSummariesAreBoundedAndSafe()
    {
        var tasks = new FakeTaskQueryService
        {
            ...
            Recent = Enumerable.Range(0, 20)
                .Select(index => TaskRun.Create(
                    index == 0
                        ? @"C:\Private\stdout token=secret-value"
                        : $"Task {index}",
                    "chat")
                    with
                    {
        ...
        Assert.DoesNotContain(dashboard.ActiveTasks.Concat(dashboard.RecentTasks), item =>
            item.SafeSummary.Contains("stdout", StringComparison.OrdinalIgnoreCase) ||
            item.SafeSummary.Contains("C:\\", StringComparison.Ordinal) ||
            item.SafeSummary.Contains("secret-value", StringComparison.Ordinal));
    }
